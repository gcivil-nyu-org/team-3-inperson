...
class TestSignupView(TestCase):
    def setUp(self):
        self.url = reverse("profiles:signup")

    def test_unauthenticated_user(self):
        user = User.objects.create(email="testemail@gmail.com", password="password1")
        self.client._login(user)
        response = self.client.get(self.url)
        expected_url = reverse("profiles:home")  # change expected_url in your project
    ...
    def test_post_method_with_valid_data(self):
        ...
        self.assertEqual(
            mail.outbox[0].from_email, "<youremail>"
...
class TestActivateAccountView(TestCase):
    def setUp(self):
        self.user = User.objects.create(
            email="testuser@gmail.com", password="password1", is_active=False
        )
    ...
    def test_test_with_valid_token(self):
        ...
        url = reverse("profiles:activate", args=[uidb64, token])

        response = self.client.get(url, follow=True)
        message = list(response.context.get("messages"))[0]
        self.assertRedirects(response, reverse("profiles:login"), 302)
        self.assertEquals(message.tags, "success")
        self.assertEquals(
...
class TestLoginView(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="testemail@gmail.com", password="password1", is_active=True
        )
        self.user2 = User.objects.create_user(
            email="testemail2@gmail.com", password="password2", is_active=False
        )
...
class TestLogoutView(TestCase):
    ...
    def test_logout(self):
        self.client._login(self.user)
        response = self.client.get(self.url)
        expected_url = reverse("profiles:login")
        self.assertRedirects(response, expected_url, 302)
...
class TestUserAccounts(TestCase):
    ...
    def test_activating_user(self):
        """Activating user account using link in the email"""
        activation_key = EmailActivation.objects.get(email=self.email).key
        user_email_activation_status = EmailActivation.objects.get(
            email=self.email
        ).activated
        user = User.objects.get(email=self.email).is_active
        ...
        # Activating the account with get request to email/confirm/<key>
        activation = self.client.get(
            reverse("profiles:email-activate", kwargs={"key": activation_key})
        )
        print(activation)
